...
// ---------------------------------------------------------------------------
//  PinkTrail - светящийся розовый шлейф за системным курсором Windows.
...
static class Cfg
{
    ...
    public static Color Core = Color.FromArgb(255, 190, 230);
    public static Color Mid  = Color.FromArgb(255, 105, 190);
    public static Color Glow = Color.FromArgb(235,  40, 140);
    ...
    public static void Save(string path)
    {
        ...
        w.WriteLine("; PinkTrail - настройки шлейфа. Файл читается при запуске.");
...
class TrayContext : ApplicationContext
{
    ...
    public TrayContext()
    {
        ...
        ContextMenu menu = new ContextMenu(new MenuItem[] {
            pauseItem,
            new MenuItem("Настройки (файл PinkTrail.ini)", new EventHandler(OnOpenIni)),
            new MenuItem("-"),
        ...
        icon = new NotifyIcon();
        icon.Icon = MakeIcon();
        icon.Text = "PinkTrail - шлейф за курсором";
    ...
    static Icon MakeIcon()
    {
        ...
        using (Graphics g = Graphics.FromImage(b))
        {
            ...
            using (Pen p = new Pen(Color.FromArgb(120, 235, 40, 140), 9f))
                g.DrawEllipse(p, 6, 6, 20, 20);
            using (Pen p = new Pen(Color.FromArgb(255, 255, 130, 200), 4f))
                g.DrawEllipse(p, 6, 6, 20, 20);
        }
        IntPtr h = b.GetHicon();
    ...
    void OnPause(object s, EventArgs e)
    {
        ...
        icon.Text = form.Paused ? "PinkTrail - пауза" : "PinkTrail - шлейф за курсором";
    }
...
static class Program
{
    ...
    static void Main()
    {
        ...
        IniPath = Path.Combine(dir, "PinkTrail.ini");
        Cfg.Load(IniPath);

        bool created;
        Mutex mutex = new Mutex(true, "PinkTrailSingleInstance", out created);
